...
def points_inside_contour(points, contour):
    """
    Check if any point in points array is inside contour
    @Param points array of points to check
    @Param contour to check against
    @Return true if any point is inside contour
    """
    for point in points:
        # Convert numpy array point to tuple format that pointPolygonTest expects
        pt = (int(point[0][0]), int(point[0][1]))

        # Check if point is inside or on the contour (using -1 for outside, 0 for on, 1 for inside)
        if cv2.pointPolygonTest(contour, pt, False) >= 0:
            return True
    return False


def remove_walls_not_in_contour(boxes, contour):
    """
    Remove walls not inside contour
    ...
    ret_boxes = []
    for box in boxes:
        # Pass entire box points array to check
        if points_inside_contour(box, contour):
